...
- [rein](https://github.com/SalzDevs/rein) — A Go library + CLI that gives AI coding agents the shell-execution layer they need: process group isolation (no leaked children on agent crash), graceful SIGTERM→SIGKILL shutdown, PTY allocation, idle-timeout detection, and overflow policies for long-running dev commands. Speaks NDJSON over stdio with Python and Node clients. Cross-platform (Linux, macOS, Windows non-PTY). MIT.
